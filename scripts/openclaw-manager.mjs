...
function runRefreshCycle() {
  ...
  return {
    ...
    mode: process.env.NEWSROOM_PULL_URL || process.env.OPENCLAW_NEWSROOM_URL ? "external-feed" : "curated-rss",
    output: compactText(result.stdout),
    warnings: compactText(result.stderr)
  };
}

function buildManagerSnapshot({ startedAt, finishedAt, refresh, submissionReview, contentPath, platformStatePath }) {
